...
public class Checker {
    ...
    public boolean check(String expression) {
        ...
        int i = 0;
        int mode = 4+16+64;
        return check(chars,i,mode, 0);
    }

    private boolean check(char[] chars, int i, int mode, int openHooks) {
        ...
        if (i >=chars.length) {
            ...
            if (contains(closeHook,chars[i])) {
                return check(chars, i+1, getCharMode(chars[i]), openHooks-1);
            }
            if (contains(openHook, chars[i])) {
                return check(chars, i+1, getCharMode(chars[i]), openHooks+1);
            }
            return check(chars, i+1, getCharMode(chars[i]), openHooks);
        }
    }
    ...
    public String removeBlanks(String s) {
        ...
        for (char c : s.toCharArray()) {
            if (Character.isDigit(c) || c=='.' || contains(hooks,c) || contains(mathOperations,c)) {
                sb.append(c);
            }
        }
        return sb.toString();
